...
Análisis de variables dependientes categóricas con dos categorías (**dicotómicas**), o más (**polinómicas**). Sirve para modelar la probabilidad de ocurrencia de un evento como función de otros factores, y responder preguntas como:
...
Este código lo hace con la media de ideología en la muestra es vez de con ideología = 5, para todos los recuerdos de voto, que esté en la media de edad de la muestra, que sea hombre, con estudios y con una opinión negativa para la economía.
...
La última tranfomación de la ecuación nos deja despejado $p(x)$, de esta manera podemos hacer probabilidades predichas de x sustituyendo los betas y las x (como hacías valores predichos en lm). Como la distribución de la muestra no es lineal los efectos de x no son iguales a lo largo de todo y, de esta manera podemos calcular casos específicos de y. Podemos conocer a lo largo de la regresión la probabilidad de y para cada x seleccionada por nosotros.

Por último podemos carcular los *Average* *Marginal* *Effects* (*margins* en Stata). El efecto marginal no es la probabilidad de ocurrencia de x, sino el fecto de la VI sobre la probabilidad de ocurrencia de x. Es la manera de conseguir el equivalente a los coeficientes de la regresión lineal porque calcual todas las y para ciertos valores de x y luego hace el promedio de esos impactos de la x. En el caso de lineales no hace falta tranformar nada porque la media del efectos de una VI sobre todos los valores de y es igual a el efecto de de VI sobre un solo valor de y porque es un efecto estable a lo largo de toda la regresión.
...
En primer lugar, creamos el conjunto de entrenamiento (60%) y test (40%). Ha cambiado los porcentajes porque hay pocos 1 de la variable dependiente que es dicotómica, y así puede coger más unos. Se saca una muestra aleatoria y por eso ponermos el seed, para que nos escoja los mismos datos aleatorios y sea replicable. Llama clase real a lo que antes era la parte de test de la VD en las predicciones de lm.
...
Repetimos el proceso con el algoritmo [Naive Bayes](https://www.r-bloggers.com/2021/04/naive-bayes-classification-in-r/) (misma filosofía que un modelo bayesiano pero no lo es, es otro modelo de clasificación).
