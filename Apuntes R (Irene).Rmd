...
library(tidyverse)
library(pacman)
source("source/Limpieza_de_datos.R")
datos<- cis2023
```
...
***
### Opciones de texto y otros elementos
**Formatos de texto:**
...
### Índices y tablas de contenido {.tabset .tabset-fade .tabset-pills}
#### Índice como lista al comienzo del documento
...
#### Índice a la izquierda del documento
...
## Dataframes {.tabset .tabset-fade .tabset-pills}

### Ficheros de texto plano (.csv, .txt, tsv, etc)
Abrir la librería readr: `library(readr)`.
...
***
### Visualizar el contenido de un dataframe
...
Crear un nuevo data.frame con una selección de columnas.
```{r
...
```{r
table(Datos$clase, useNA="ifany") 
```
La opción useNA="ifany" es opcional, y se añade para que en la tabla se muestren también los valores perdidos.
...
***
### Añadir etiquetas a la variable
...
### Comparación de medias (T-test) {.tabset .tabset-fade .tabset-pills}

#### En R

**Test de una media**: 

El objetivo de este tipo de test es comprobar si la media poblacional de una variable se corresponde con una determinada cifra.Variables continuas. 
```r
...
**Comparación de medias en muestras INDEPENDIENTES: dos variables numéricas**

Si las dos variables son numéricas, aplicamos el siguiente código
```{r}
# t.test(y1, y2) (donde y1 e y2 son variables numéricas)
```


**Muestras DEPENDIENTES: dos variables numéricas**
```r
...
<center>
![Intervalos de confianza](imagenes/intervalo_conf.png){width=320px}
</center>
...
***
### Análisis de correlación (var. continuas)  {.tabset .tabset-fade .tabset-pills}
...
La funión para calcular la correlación es `cor()`. Sin embargo, para poder interpretar más adecuadamente los resultados de la correlacion conviene realizar un test para comprobar si dicha correlación es estadísticamente significativa. Las hipótesis de este test son:
...
```{r include=FALSE}
#La base de datos mtcars, que viene incluída con R.
attach(mtcars)
Data <- mtcars
```
...
***
### Tablas de contingencia (var. categóricas) {.tabset .tabset-fade .tabset-pills}
...
**Otras tablas: xtabs vs CrossTable** (mejor usar `xtabs()`):

```r
tabla2 <- xtabs(~var1+var2, data = datos) 
ftable(tabla2) 
...
```
  ...
  iii. En nuestro caso, se cumple el supuesto.
```{r}
...
5. **Multicolinealidad**. Hace referencia a la correlación entre las VIs. Podemos medir la existencia de multicolinealidad usado el **VIF** (Variation Inflation Factor).
```{r}
p_load(rms)
vif(reg.lineal)
```
...
1. [Preparar la base de datos (eliminar NA)](#)
2. [Modelo de regresión y resultados](#)
3. [Visualización de los coeficientes](#)
4. [Supuestos del modelo](#)
5. [Valores predichos e intervalos de confianza](#)
...
```{r}
data1 <- data.frame(hombre="Hombre", edad=50, estudios_universitarios="con ES", ecoper="positiva", ecoesp="negativa", ideol=5, recuerdo19="Ciudadanos")
yhat1<-predict(modelPP, newdata = data1)
yhat1
```
```{r}
levels(modelPP$model$estudios_universitarios)
```
...
```{r}
data2<- data.frame(hombre="Hombre", edad=c(20, 30, 40, 50, 60, 70, 80), estudios_universitarios="con ES", ecoper="positiva", ecoesp="negativa", ideol=5, recuerdo19="Ciudadanos")
...
export_summs(modelPP, modelPSOE, 
  ...
  coefs=c("Edad"="edad", 
          "Hombre"="hombreHombre", 
          "Estudios Universitarios"="estudios_universitarioscon ES", 
